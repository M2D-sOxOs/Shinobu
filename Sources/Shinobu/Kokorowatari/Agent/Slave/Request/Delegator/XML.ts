...
import { XML as XMLRule } from "../../../Rule/Platform/Command/XML";
import { Result } from "../../../Rule/Platform/Command/XML/Result";
...
export class XML extends Delegator {

  private _XML!: XMLRule;
  ...
  public async Initialize(): Promise<XML> {

    await super.Initialize();

    this._XML = this.Command.XML!;

    if (!this._Client || !this._Request) Urusai.Panic('Client / Request is required in XML.');
  ...
  protected async _PerformResult(scopeZone: any): Promise<boolean> {
    ...
    if (!(await this._XML.Indicator.Estimate(this.Session, scopeZone))) {
      Urusai.Warning('Result is not passing indicator exam');
      return false;
    }

    scopeZone['__RESULT__'] = (this.FlowZone['__RESULT__'] = await this.__PerformResultStructure(scopeZone, this._XML.Result));
    if (this._XML.Postprocess) return await this._XML.Postprocess.Value(this.Session, scopeZone);
    return true;
  }
    ...
    switch (result.Type) {
      ...
      case 'ARRAY':
        ...
        if (searchArray instanceof Array) {
          ...
          for (const v in searchArray) {
            scopeZone[result.MapTo] = searchArray[v];
            resultArray.push(await this.__PerformResultStructure(scopeZone, result.Value as Result));
          }
        }
        return resultArray;
