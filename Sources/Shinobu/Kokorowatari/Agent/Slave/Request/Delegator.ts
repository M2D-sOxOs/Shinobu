...
export abstract class Delegator {
  ...
  constructor(public readonly Command: Command, public readonly Session: any, public readonly AdditionalZones: any) {
  ...
  public async Perform() {
    ...
    try {
      ...
      return (await Tokei.Record('Network-Request', async () => {
        ...
        return await this._PerformResult();
      }));
    } catch (e) {
      console.log(e);
  ...
  protected abstract _PerformRequest(): Promise<boolean>;

  protected abstract _PerformResult(): Promise<boolean>;
  ...
  protected async _Inflate(expressionTable: Table<Expression>): Promise<Table<string>> {
    ...
    for (const key in expressionTable) {
      if (false === (inflatedTable[key] = await expressionTable[key].Value(this.Session, this.AdditionalZones))) {
